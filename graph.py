...
def graph(file_name):
    ...
    sorted_data = sorted(ticker_count.items(), key=lambda x: x[1], reverse=True)
    labels = [f"{ticker} ({position})" for (ticker, position), count in sorted_data]
    counts = [count for _, count in sorted_data]
    colors = ['red' if position == 'P' else 'blue' for (ticker, position), _ in sorted_data]

    plt.figure(figsize=(10, 6))
    plt.bar(labels, counts, color=colors)
    plt.xlabel("Ticker (Position)", fontsize=10)
    plt.ylabel("Count", fontsize=10)
    plt.title("Count of Tickers by Position", fontsize=13)
